...
std::shared_ptr<Tensor> SoftMaxLayer::backward(const std::shared_ptr<Tensor>& dOut) {
	...
	std::shared_ptr<Tensor> tmp = std::make_shared<Tensor>(m, n);
	cudnnHandle_t* handle = handlers->getCudnnHandle();
	checkCUDNN(
		cudnnSoftmaxBackward(*handle, CUDNN_SOFTMAX_ACCURATE, CUDNN_SOFTMAX_MODE_CHANNEL, &alpha, outTensor->desc, outTensor->d_data, dOut->desc, dOut->d_data, &beta, tmp->desc, tmp->d_data)
	);
	return tmp;
};
